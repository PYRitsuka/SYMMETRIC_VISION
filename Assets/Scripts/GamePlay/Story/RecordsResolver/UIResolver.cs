...
namespace GamePlay.Story.RecordsResolver
{
    ...
    public class UIResolver : MonoBehaviourSingleton<UIResolver>, IResolvable
    {
[SerializeField] private RawImage pictureArchetype;
        ...
        public RotatedImageDisplay rotatedImageDisplay;
        ...
        // private IEnumerator BlendToWhite(Material material,float duration)
        // {
        //     float time = 0;
        //     //float initialBrightness = material.GetFloat("_Brightness");

        //     while (time < duration)
        //     {
        //         time += Time.deltaTime;
        //         float blendAmount = time / duration;
        //         material.SetFloat("_Brightness", Mathf.Lerp( 1.0f,0.0f, blendAmount));
        //         //material.SetFloat("_Contrast", Mathf.Lerp( 1.0f,0.0f, blendAmount));
        //         yield return null;
        //     }

        //     // Ensure the final brightness is set to maximum
        //     //material.SetFloat("_Brightness", initialBrightness);
        // }
        ...
        void CutInOut()
        {
            ...
            else if (pat.Contains("VIEW展开")) {
                Debug.Log($"VIEWWWWW {maskGo.sizeDelta}");
                maskGo.sizeDelta = new Vector2(0, 0);
                maskGo.DOSizeDelta(new Vector2(0, 0), duration).SetDelay(1);

                var img =  _selectedTransform.GetComponent<RawImage>();
                // viewFrame.isLoading = true;
                if (pat.Contains("粉")) {
                    viewFrame.current_color = ViewFrame.FrameColor.Pink;
                } else if (pat.Contains("灰")) {
                    viewFrame.current_color = ViewFrame.FrameColor.Gray;
                } else if (pat.Contains("蓝")) {
                    viewFrame.current_color = ViewFrame.FrameColor.Blue;
                }
                viewFrame.SetState(0);
                viewFrame.imgObject.texture = img.texture;
                viewFrame.DoExpand(duration);

            }else if (pat.Contains("VIEW切换")) {
                maskGo.sizeDelta = new Vector2(0, 0);
                var img =  _selectedTransform.GetComponent<RawImage>();
                //viewFrame.imgObject.texture = img.texture;
                viewFrame.SetTexture(img.texture,duration);
            }
            else if (pat.Contains("VIEW收起")) {
                viewFrame.DoClose(duration);
            } 
            else if (pat.Contains("框体展开") ) {
                maskGo.sizeDelta = new Vector2(0, 0);
                maskGo.DOSizeDelta(new Vector2(0, 0), duration).SetDelay(1);
                var img =  _selectedTransform.GetComponent<RawImage>();;
                rotatedImageDisplay.gameObject.SetActive(true);
                if (pat.Contains("粉")) {
                    rotatedImageDisplay.current_color = RotatedImageDisplay.FrameColor.Pink;
                } else if (pat.Contains("灰")) {
                    rotatedImageDisplay.current_color = RotatedImageDisplay.FrameColor.Gray;
                } else if (pat.Contains("蓝")) {
                    rotatedImageDisplay.current_color = RotatedImageDisplay.FrameColor.Blue;
                }
                rotatedImageDisplay.imgObject.texture = img.texture;
                rotatedImageDisplay.DoExpand(duration);

            } else if (pat.Contains("框体切换")) {
                maskGo.sizeDelta = new Vector2(0, 0);
                var img =  _selectedTransform.GetComponent<RawImage>();
                //rotatedImageDisplay.imgObject.texture = img.texture;
                rotatedImageDisplay.SetTexture(img.texture,duration);
            } else if (pat.Contains("框体收起") ) {
                rotatedImageDisplay.DoClose(duration);
            } else
            {
                if (pat.Contains("展开"))
                {
        ...
        protected static string GetFolderName() => "UI";
    }
}